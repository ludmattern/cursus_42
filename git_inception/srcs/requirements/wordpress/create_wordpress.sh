...
then
    ...
    php -r "file_put_contents('wordpress-6.4.tar.gz', file_get_contents('https://wordpress.org/wordpress-6.4.tar.gz', false, stream_context_create(['ssl' => ['verify_peer' => false, 'verify_peer_name' => false]])));"
    tar xfz wordpress-6.4.tar.gz
    mv wordpress/* .
