...
/*   Updated: 2024/07/27 17:24:21 by lmattern         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
bool RPN::isValidNumber(const std::string& token) const {
    if (token.length() == 1 && isdigit(token[0])) return true;
    if (token.length() == 2 && token[0] == '-' && isdigit(token[1]) && token[1] >= '1' && token[1] <= '9') {
        return true;
    }
    return false;
}
